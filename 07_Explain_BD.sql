...
INSERT INTO usuarios (nome, e_mail, data_nascimento, rua) VALUES
...
-- O EXPLAIN no Postgres te dirá se ele está fazendo um Seq Scan (lendo a tabela toda linha por linha) ou um
-- Index Scan (indo direto no alvo).

-- Analisando a busca simples
EXPLAIN SELECT * FROM usuarios WHERE nome = 'Maria Santos';

-- Analisando a busca com JOIN
EXPLAIN SELECT * FROM usuarios us
INNER JOIN reservas rs
ON us.id = rs.id_usuario
WHERE us.nome = 'Maria Santos';

-- O índice funciona como o índice remissivo de um livro jurídico: você não precisa ler o livro todo para achar um termo.

-- Criando o índice na coluna nome
CREATE INDEX idx_nome ON usuarios (nome);

-- Testando novamente para ver a diferença no plano de execução
EXPLAIN SELECT * FROM usuarios WHERE nome = 'Maria Santos';
